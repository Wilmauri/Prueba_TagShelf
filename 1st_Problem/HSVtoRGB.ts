...
interface RGB {
  ...
  function testHsvToRgbConversion(): void {
    const testColors: HSV[] = [
      { h: 0, s: 1, v: 1 },       // Red
      { h: 120, s: 1, v: 1 },     // Green
      { h: 240, s: 1, v: 1 },     // Blue
      { h: 60, s: 1, v: 1 },      // Yellow
    ...
    testColors.forEach((color: HSV) => {
      const rgb: RGB = HSVtoRGB(color.h, color.s, color.v);
      console.log(
        `HSV(${color.h.toFixed(1)}°, ${color.s.toFixed(2)}, ${color.v.toFixed(2)}) → ` +
